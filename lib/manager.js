...
const NeeoSdkHelper = new NeeoSdkWebunity('ziggo-horizon:manager');
...
class Manager extends EventEmitter {
	...
	constructor() {
		...
		this.ssdpEnabled = NeeoSdkHelper.ConfigHasOr('ziggoHorizon.Discovery.SsdpEnabled', false);
		this.ssdpTimeout = NeeoSdkHelper.ConfigHasOr('ziggoHorizon.Discovery.SsdpTimeout', 15) * 1000;
		this.ssdpRediscovery = NeeoSdkHelper.ConfigHasOr('ziggoHorizon.Discovery.SsdpRediscovery', -1);
		this.knownMediaboxes = NeeoSdkHelper.ConfigHasOr('ziggoHorizon.Discovery.KnownMediaboxes', []);
	}
	...
	_BoxDiscovered(serialNumberOrUniqueId, modelName, modelDescription, ipAddress) {
		...
		// Already added box
		NeeoSdkHelper.Debug('- Found a ' + modelName + ' (' + modelDescription + ') with serial number \'' + serialNumberOrUniqueId + '\' at IP \'' + ipAddress + '\'');
		if (serialNumberOrUniqueId in this.mediaBoxes) {
			if (this.mediaBoxes[serialNumberOrUniqueId].getIp() === ipAddress) {
				NeeoSdkHelper.Debug('  - Skipped mediabox with uniqueId \'' + modelDescription + '\': already known with this IP address.');
			} else {
				NeeoSdkHelper.Debug('  - Skipped mediabox with uniqueId \'' + modelDescription + '\': already added with name \'' + modelDescription + '\' and IP \'' + ipAddress + '\'.');
			}
			return;
		}
	...
	_DiscoverBoxes() {
		...
		return new Promise(function (resolve, reject) {
			...
			if (!_this.ssdpEnabled) {
				NeeoSdkHelper.Debug('SSDP discovery disabled, therefore we seem to be done...');
				resolve();
				return;
			}
			...
			const ssdpTimeoutTimer = setTimeout(function () {
				...
				ssdpClient.stop();
				NeeoSdkHelper.Debug('SSDP discovery finished.');
			...
			ssdpClient.on('response', function (headers, statusCode, rinfo) {
				...
				http.get(headers['LOCATION'], function (res) {
					...
					if (error) {
						res.resume();
						NeeoSdkHelper.Debug('HTTP_GET_ERROR: ' +  error.message);
					...
					res.on('end', function () {
						try {
							...
							NeeoSdkHelper.Debug('XML_EXCEPTION: ' +  e.message);
							NeeoSdkHelper.Debug(rawData);
							_this.emit('SsdpDiscoveryException', e.message, rawData);
						}
					});
				}).on('error', function (e) {
					NeeoSdkHelper.Debug('HTTP_GET_ERROR: ' +  e.message);
	...
	LoadAndDiscoverBoxes() {
		...
		return new Promise(function (resolve, reject) {
			...
			if (NeeoSdkHelper.IsArray(preConfiguredMediaboxes) && (preConfiguredMediaboxes.length > 0)) {
				...
				if (!preConfiguredMediaboxes.every(item => item.hasOwnProperty('uniqueId') && item.hasOwnProperty('modelName') && item.hasOwnProperty('modelDescription') && item.hasOwnProperty('ipAddress'))) {
					NeeoSdkHelper.Debug('ERROR: Make sure each object in the \'ziggoHorizon.PreConfiguredMediaboxes\' array has a unique id (or serial number), a label and an ip; e.g. { "uniqueId": "someUniqueValue", "label": "Some descriptive label", "ip": "127.0.0.1" }')
				} else {
	...
	ButtonPressed(serialNumberOrUniqueId, button) {
		serialNumberOrUniqueId = serialNumberOrUniqueId.toString();
		if (!(serialNumberOrUniqueId in this.mediaBoxes)) {
			NeeoSdkHelper.Debug('- Unable to send button to device with unique id (or serial number) "' + serialNumberOrUniqueId + '". It appears to be missing from the list...');
			return;
		}
	...
	FavoritePressed(serialNumberOrUniqueId, favoriteId) {
		serialNumberOrUniqueId = serialNumberOrUniqueId.toString();
		if (!(serialNumberOrUniqueId in this.mediaBoxes)) {
			NeeoSdkHelper.Debug('- Unable to send favorite to device with unique id (or serial number) "' + serialNumberOrUniqueId + '". It appears to be missing from the list...');
			return;
		}

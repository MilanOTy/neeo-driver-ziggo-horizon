...
class Manager extends EventEmitter {
	...
	ButtonPressed(serialNumberOrUniqueId, button) {
		serialNumberOrUniqueId = serialNumberOrUniqueId.toString();
		if (!(serialNumberOrUniqueId in this.mediaBoxes)) {
			Helper.Debug('- Unable to send button to device with unique id (or serial number) "' + serialNumberOrUniqueId + '". It appears to be missing from the list...');
			return;
		}
	...
	FavoritePressed(serialNumberOrUniqueId, favoriteId) {
		serialNumberOrUniqueId = serialNumberOrUniqueId.toString();
		if (!(serialNumberOrUniqueId in this.mediaBoxes)) {
			Helper.Debug('- Unable to send favorite to device with unique id (or serial number) "' + serialNumberOrUniqueId + '". It appears to be missing from the list...');
			return;
		}

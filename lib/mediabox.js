 ...
 */
class Mediabox {
	constructor(modelDescription, ipAddress) {
		// Settings
	...
	connect() {
		var _this = this;
		this.debug('Connecting to ' + this.getIpAddress() + ' (max. 10 seconds) ...');
		...
		this.socket.on('data', function (data) {
			...
			switch (_this.connectionState) {
				...
				case CONSTANTS.CONNECTION_STATE.CONNECTING:
					if (datastring == "\u0001\u0001") {
						_this.debug('Received OK message, returning: ' + _this.Hex2Bin('01'));
